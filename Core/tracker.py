...
class Tracker:
    ...
    def __init__(
        self, server, submit=None, dir_path="", verbose=False, super_verbose=False
    ):
        self.server = server
        ...
        self._squelch_table_event = (
            0  # counter: suppress this many upcoming TABLE events
        )
        self._selected_index = None
        self._pending_timers = {}  # condition → expiry turn, set during the current edit session
        self._connected_players: dict[
            str, dict
        ] = {}  # name → {"select": bool, "move": bool, "aoe": bool, "color": str}
        self._known_players: dict[
            str, str
        ] = {}  # name → color; persists across disconnects
    ...
    def _load_condition_images(self) -> dict:
        ...
        for cond in self.condition_list:
            png_path = os.path.join(
                self.dir_path, "Assets", "Conditions", f"{cond}.png"
            )
            ...
            if _PIL_OK:
                ...
                img = img.resize(
                    (CONDITION_ICON_SIZE, CONDITION_ICON_SIZE), Image.LANCZOS
                )
                buf = io.BytesIO()
    ...
    def _sync_map_ui(self, map_path, map_visible):
        ...
        if has_map:
            self.window["Toggle Map"].update(
                text="Hide Map" if map_visible else "Show Map"
            )
    ...
    def build_gui_layout(self):
        ...
        layout = [
            ...
            [
                sg.Table(
                    values=[],
                    headings=["Name", "Initiative", "HP", "Notes"],
                    auto_size_columns=False,
                    justification="left",
                    col_widths=[10, 10, 10, 40],
                    key="-TABLE-",
                    enable_events=True,
                    row_height=36,
                    expand_x=True,
            ...
            [
                sg.Text("Turn:", font=table_font),
                sg.Input(str(self.server.turn), key="-TURN-", size=(5, 1)),
                sg.Button("⏮ Prev Char"),
                sg.Button("⏭ Next Char"),
            ],
            [sg.HorizontalSeparator()],
            [
                sg.Text("Name", size=(10, 1), font=table_font),
                sg.Input(key="-NAME-", size=(30, 1)),
            ],
            [
                sg.Text("Initiative", size=(10, 1), font=table_font),
                sg.Input(key="-INITIATIVE-", size=(5, 1)),
            ],
            [
                sg.Text("HP:", size=(10, 1), font=table_font),
                sg.Input(key="-HP-", size=(5, 1)),
                sg.Text("Max HP:", font=table_font),
                sg.Input(key="-MAX_HP-", size=(5, 1)),
                sg.Text("   Change:", font=table_font),
                sg.Input("0", key="-HP_CHANGE-", size=(5, 1)),
                sg.Button("Wound"),
                sg.Button("Heal"),
            ],
            [
    ...
    def refresh_table(self, selected_index=None):
        ...
        for i, c in enumerate(self.server.combatants):
            ...
            timer_parts = [
                f"{_COND_ABBREV.get(cond, cond[:3])}:{exp[0]}@{exp[1]}"
                for cond, exp in sorted(c.condition_timers.items())
            ]
            notes_display = c.notes + (
                " [" + ", ".join(timer_parts) + "]" if timer_parts else ""
            )
            data.append(
                [name, c.initiative, "" if c.hp is None else c.hp, notes_display]
            )
        ...
        if selected_index is not None and 0 <= selected_index < len(
            ...
            self._squelch_table_event = (
                2  # update clears then reselects — two TABLE events expected
            )
        ...
        for info in self._connected_players.values():
            ...
            if rgb:
                r, g, b = rgb
                dimmed = (
                    f"#{(r + 255) // 2:02x}{(g + 255) // 2:02x}{(b + 255) // 2:02x}"
                )
                tree.tag_configure(f"pc_{c}", background=dimmed, foreground="black")
        new_photos = {}
        for item_id, conditions, cname in zip(
            tree.get_children(), row_conditions, row_names
        ):
        ...
        if event == "-TABLE-":
            try:
                if values["-TABLE-"]:
                    ...
                    if row_index == 0:
                        ...
                        if self.verbose:
                            log_msg(
                                f"[Tracker] Selected index = {self._selected_index}, Combatant: {c}"
                            )
                        self.window["-NAME-"].update(c.name)
                        self.window["-INITIATIVE-"].update(c.initiative)
                        self.window["-HP-"].update("" if c.hp is None else c.hp)
                        self.window["-MAX_HP-"].update(
                            "" if c.max_hp is None else c.max_hp
                        )
                        self.window["-IS_PC-"].update(c.is_pc)
                        self.window["-SIZE-"].update(c.size)
                        for cond in self.condition_list:
            ...
            if values.get(event):  # condition turned ON
                ...
                popup_layout = [
                    ...
                    [
                        sg.Text(
                            "(leave Rounds blank for permanent)", font=(_UI_FONT, 10)
                        )
                    ],
            ...
            c = self.server.combatants[self._selected_index]
            new_conditions = [
                cd for cd in self.condition_list if values.get(f"-COND_{cd}-")
            ]
            new_timers = {
                cd: t for cd, t in c.condition_timers.items() if cd in new_conditions
            }
            ...
            try:
                ...
                fields = {
                    ...
                    "hp": int(values["-HP-"].strip())
                    if values["-HP-"].strip()
                    else None,
                    "max_hp": int(values["-MAX_HP-"].strip())
                    if values["-MAX_HP-"].strip()
                    else None,
                    "is_pc": values.get("-IS_PC-", False),
                    "size": max(1, int(size_str)) if size_str.isdigit() else 1,
                }
                ...
                return
            self._submit(
                {"action": "update_combatant", "name": old_name, "fields": fields}
            )
            ...
            self._submit(
                {"action": "set_map_visible", "visible": not self._map_visible}
            )

        elif event == "-SIGHT_RADIUS-":
            self._submit(
                {
                    "action": "set_visibility_radius",
    ...
    def run_gui(self, dir_path):
        ...
        self.window = sg.Window(
            "D&D Initiative Tracker", layout, resizable=True, finalize=True
        )
        ...
        while True:
            ...
            if win == self.window:
                if event == "-CLOSE_REQUESTED-":
                    if (
                        sg.popup_yes_no(
                            "Are you sure you want to quit DungeonPy?",
                            title="Quit",
                            keep_on_top=True,
                        )
                        == "Yes"
