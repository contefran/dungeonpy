...
FALLBACK_COLORS = {
    ...
    IRON_DOOR: (100, 100, 120),
    SECRET_DOOR: (60, 60, 70),
    TRAP: (80, 60, 60),
    GRASS: (60, 120, 40),
}
...
class MapEditor:
    def __init__(self, dir_path, initial_file=None):
        ...
        pygame.display.set_caption(
            ...
            if self.save_path
            else "DungeonPy Map Editor — unsaved"
        )
    ...
    def _rescale_textures(self):
        ...
        self._tex = {
            ...
            for t, orig in self._tex_orig.items()
            if orig is not None
        }
    ...
    def _blit_tile(self, screen, tile, x, y):
        ...
        if tile in self._tex:
            ...
            pygame.draw.rect(
                screen, FALLBACK_COLORS.get(tile, (80, 80, 80)), (x, y, ts, ts)
            )
        ...
        if self._paint_mode == "rect" and self._rect_start and self._rect_end:
            ...
            for rr in range(rmin, rmax + 1):
                for cc in range(cmin, cmax + 1):
                    screen.blit(
                        fill, (cc * ts + self.offset_x, rr * ts + self.offset_y)
                    )
            ...
            pygame.draw.rect(
                screen,
                (255, 220, 80),
                (px, py, (cmax - cmin + 1) * ts, (rmax - rmin + 1) * ts),
                2,
            )
        else:
    ...
    def _draw_toolbar(self, screen):
        ...
        for i, (tile, label) in enumerate(TILE_LABELS):
            ...
            if tile != VOID and tile in self._tex_orig and self._tex_orig[tile]:
                screen.blit(
                    pygame.transform.scale(self._tex_orig[tile], (sw, sw)), (sx, sy)
                )
        ...
        pygame.draw.line(
            screen, (65, 65, 75), (margin, sep_y), (TOOLBAR_W - margin, sep_y)
        )
        ...
        for mi, (mode_id, mode_lbl) in enumerate([("free", "Free"), ("rect", "Rect")]):
            ...
            screen.blit(
                ms,
                (
                    mrect.x + (mrect.w - ms.get_width()) // 2,
        ...
        sep2_y = mode_y + 24 + 6
        pygame.draw.line(
            screen, (65, 65, 75), (margin, sep2_y), (TOOLBAR_W - margin, sep2_y)
        )
        ...
        while True:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if event.button == 1:
                        if (
                            self._paint_mode == "rect"
                            and self._painting
                            and self._rect_start
                        ):
                            self._push_undo()
                            r0, c0 = self._rect_start
