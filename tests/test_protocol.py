import pytest
from Core.protocol import validate_intent, make_event, make_snapshot, make_error, INTENTS
...
def test_make_event_has_correct_fields():
    ...
    assert e == {"type": "event", "action": "turn_advanced", "seq": 3, "active": "Hero", "turn": 2}
...
def test_submit_multi_event_intent_uses_consecutive_seqs(server):
    ...
    received = []
    server.subscribe(received.append)
    server.submit({"action": "add_combatant", "combatant": {"name": "A", "initiative": 10}})
    server.submit({"action": "add_combatant", "combatant": {"name": "B", "initiative": 5}})
    received.clear()
    server.submit({"action": "update_combatant", "name": "B", "fields": {"initiative": 20}})
    seqs = [m["seq"] for m in received]
...
def test_submit_echoes_client_req_id_on_snapshot(server, tmp_path):
    ...
    import json; open(path, "w").write(json.dumps({"initiative": [], "active_index": 0, "turn": 1}))
    received = []
    server.subscribe(received.append)
...
def test_periodic_snapshot_fires_at_interval(server):
    ...
    for i in range(3):
        server.submit({"action": "add_combatant", "combatant": {"name": str(i), "initiative": i}})
